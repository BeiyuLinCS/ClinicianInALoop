...
def extract_func(home_name, start_date):
    # testbedname = "tm002"
    # local_start_stamp = "2017-03-01"
    # local_end_stamp = "2017-10-26"

    testbedname = home_name
    local_start_stamp = start_date
    local_end_stamp = start_date
    ...
    #print "Printing ALL data between %s and %s" % (local_start_stamp, local_end_stamp)
    #print_all_events(tb_data)
    #print_db_style_events(tb_data)

    limit_target = "BedroomABed"
    limit_sensor_type = "Control4-Motion"

    tb_limited_data = get_sensor_testbed_data(testbedname,
                                              utc_start_stamp,
                                              utc_end_stamp,
                                              limit_target,
                                              limit_sensor_type)

    #print "Printing data between %s and %s for target %s and sensor_type %s" % (local_start_stamp,
    #                                                                            local_end_stamp,
    #                                                                            limit_target,
    #                                                                            limit_sensor_type)
    #print_all_events(tb_limited_data)
