...
class AlFeatureWindowDuration(AlFeatureTemplate):
    def __init__(self,gapFileName ,normalize=False):
        """
        ...
        #self.mylist = []
        self.f = open(gapFileName,'w')
        #self.f = open("../../testRecordGAP.txt",'w')
    ...
    def get_feature_value(self, data_list, cur_index, window_size, sensor_name=None):
        ...
        if (window_duration > 3600 * 12 or window_duration < 0):
            self.is_valid = False
            ...
            if data_list[cur_index]['datetime'].month != data_list[cur_index - 1]['datetime'].month or \
                ...
                #self.mylist.append(cur_index, cur_index + 1, data_list[cur_index - 1]['datetime'].isoformat(),date_advanced, hour_advanced)
                # could you please do the write function for the above two print
                self.f.write(str(cur_index) + "\t" + str(cur_index + 1) + "\t" + str(data_list[cur_index - 1]['datetime'].isoformat()) + "\t" + str(date_advanced)+ "\t" + str(hour_advanced) +"\n")
                self.f.flush()
        if self.normalize:
    ...
    #def write_into_file(self, mylist):
